...
root = tk.Tk()
root.title(utility.References.main_win_title)
root.iconbitmap(utility.resource_direct(r"assets\icon\simple_64.ico"))
utility.set_window_center(root,size=utility.References.main_win_initial_size)

def main():
    edit_zone = tk.Text(font=utility.References.edit_zone_font)
    ...
    menu_file = tk.Menu(menu_bar,background="white",font=utility.References.ui_font,tearoff=False)
    ...
    menu_edit = tk.Menu(menu_bar,background="white",font=utility.References.ui_font,tearoff=False)
    ...
    menu_view = tk.Menu(menu_bar,background="white",font=utility.References.ui_font,tearoff=False)
    ...
    menu_bar.add_command(label="About",command=utility.about)
